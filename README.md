...
| <a name="input_location"></a> [location](#input\_location) | GCP Location for access to regional roles | `string` | `null` | no |
...
| <a name="input_service_accounts"></a> [service\_accounts](#input\_service\_accounts) | Service account to manage and link to WIF | <pre>map(object({<br/>    # WIF provider attributes. If attribute is set to `*` all identities in the pool are granted access to SAs.<br/>    #attributes = list(string)<br/>    attributes = list(object({<br/>      attribute = string<br/>      pool      = string<br/>      provider  = string<br/>    }))<br/>    bindings = list(object({<br/>      resource_id   = string<br/>      resource_type = string<br/>      location      = optional(string, null)<br/>      roles         = list(string)<br/>    }))<br/>    can_impersonate = optional(list(string), [])<br/>    description     = string<br/>    display_name    = string<br/>    #pool = string<br/>    project = string<br/>  }))</pre> | n/a | yes |
